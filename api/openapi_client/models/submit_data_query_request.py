...
class SubmitDataQueryRequest(object):
    ...
    def __init__(self, column_separator=None, compression=None, encryption_key=None, output=None, output_format=None, query=None, read_deleted=False, source_data=None, use_index_join=None, local_vars_configuration=None):  # noqa: E501
        """SubmitDataQueryRequest - a model defined in OpenAPI"""  # noqa: E501
    ...
    def encryption_key(self, encryption_key):
        ...
        if (self.local_vars_configuration.client_side_validation and
                encryption_key is not None and not re.search(r'^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$', encryption_key)):  # noqa: E501
    ...
    def output_format(self):
        ...
        Specifies the format of the query results.  * `JSON` - Each row in the query results will be a JSON object. The format of the query result file is [JSON Lines](http://jsonlines.org/). * `CSV` - Each row in the query results will be a comma-separated list of values. * `TSV` - Each row in the query results will be a tab-separated list of values. * `DSV` - Pass any character as your custom delimiter into the `columnSeparator` field.   # noqa: E501
    ...
    def output_format(self, output_format):
        ...
        allowed_values = ["JSON", "CSV", "TSV", "DSV"]  # noqa: E501
    ...
    def source_data(self):
        ...
        Specifiy that data queries run against the live transactional databases at Zuora (Data Query Live), or run against the optimized, replicated database at 12 hours freshness for high volume extraction (Data Query Unlimited).  **Note**: Data Query Unlimited is an Early Adopter feature.  If you want to have access to the feature, submit a request at [Zuora Global Support](http://support.zuora.com/).   * `DATAHUB` - Data queries run against the optimized, replicated database at 12 hours freshness for high volume extraction (Data Query Unlimited). Data Query Unlimited is an Early Adopter feature.  If you want to have access to the feature, submit a request at [Zuora Global Support](http://support.zuora.com/).  * `LIVE` - Data queries run against the live transactional databases at Zuora (Data Query Live).  The default value is `LIVE`.   # noqa: E501
    ...
    def source_data(self, source_data):
        ...
        :type: str
        """
        allowed_values = ["DATAHUB", "LIVE"]  # noqa: E501
        if self.local_vars_configuration.client_side_validation and source_data not in allowed_values:  # noqa: E501
            raise ValueError(
                "Invalid value for `source_data` ({0}), must be one of {1}"  # noqa: E501
                .format(source_data, allowed_values)
            )
